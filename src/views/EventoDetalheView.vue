...
import { diasRestantesEvento, estadoTemporalEvento, partesDataEvento, dataExtensaEvento } from '@/composables/useEventoData'
...
import EventoBannerFullscreen from '@/components/events/EventoBannerFullscreen.vue'
...
import IconExpandir from '@/components/icons/IconExpandir.vue'
...
const partes = computed(() => partesDataEvento(evento.value?.data ?? ''))
const dataExtensa = computed(() => dataExtensaEvento(evento.value?.data ?? ''))
...
<template>
  <div v-if="evento">
      ...
      <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6">
        ...
        <div class="flex flex-col gap-6 sm:flex-row sm:items-start">
          ...
          <div class="flex shrink-0 flex-col items-center justify-center gap-1 rounded-sm bg-brand px-6 py-5">
            ...
            <span class="font-mono text-[10px] text-on-brand/40">{{ partes.ano }}</span>
          </div>
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <span
                ...
                :class="{
                  ...
                  'bg-status-good-bg text-status-good': evento.status !== 'suspenso' && estado === 'hoje',
                  'bg-brass/15 text-brass-ink': evento.status !== 'suspenso' && estado === 'futuro',
                  'bg-paper-dim text-ink-soft': evento.status !== 'suspenso' && estado === 'passado',
                }"
              >
                <span v-if="evento.status !== 'suspenso' && estado === 'hoje'" class="h-1.5 w-1.5 animate-pulse rounded-full bg-status-good" aria-hidden="true" />
                {{ evento.status === 'suspenso' ? 'Suspensa' : rotuloContagem }}
              </span>
            ...
            <button
              type="button"
              class="mt-4 inline-flex items-center gap-1.5 rounded-sm border border-line bg-card px-3 py-1.5 text-xs font-semibold text-ink-soft transition hover:border-brass hover:text-brass-ink"
              @click="bannerAberto = true"
            >
              <IconExpandir class="h-3.5 w-3.5" />
              Ver banner da reunião
            </button>
          </div>
        </div>
      </div>
    ...
    <EventoBannerFullscreen v-if="bannerAberto" :evento="evento" @fechar="bannerAberto = false" />
    ...
    <div class="mx-auto flex max-w-4xl flex-col gap-10 px-4 py-10 sm:px-6">
      ...
      <section v-if="evento.assuntos && evento.assuntos.length > 0" class="flex flex-col gap-4">
        <h2 class="font-display text-lg font-bold text-ink">
          Pauta
          <span v-if="evento.status === 'suspenso'" class="font-body text-sm font-normal text-ink-soft">
            — prevista, não chegou a ser discutida
          </span>
        </h2>
      ...
      <!-- Reunião suspensa: não chegou a acontecer — sem formulário e sem "confirmaram presença" -->
      <section v-else-if="evento.status === 'suspenso'" class="panel border-status-critical/30 p-5">
        <p class="font-mono text-[11px] uppercase tracking-widest text-status-critical">Suspensa</p>
        <p class="mt-2 text-sm text-ink-soft">
          Esta reunião foi suspensa e não chegou a acontecer nesta data — sem inscrições abertas.
        </p>
      </section>
